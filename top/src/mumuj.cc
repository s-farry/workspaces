...
int main(int argc, char* argv[]) {
  ...
  while ( maxEvts == -1 || iEvent < maxEvts ) {
    ...
    if (jets.size() > 0) {
      ...
      if (mups.size() > 0) jet_dr = get_dR( mups.at(0).p(), jets[0]);
      if (mums.size() > 0)  jet_dr  = min(jet_dr, get_dR( mums.at(0).p(), jets[0]));
    }
    ...
    if (fwdjets.size() > 0) {
      ...
      if (mups.size() > 0) fwdjet_dr = get_dR( mups.at(0).p(), fwdjets[0]);
      if (mums.size() > 0) fwdjet_dr = min(fwdjet_dr, get_dR( mums.at(0).p(), fwdjets[0]));
    }
    if (bwdjets.size() > 0) {
      getVals(bwdjets[0], outJets["bwdjet"], hard_partons, mesons, pythia.event);
      if (mups.size() > 0) bwdjet_dr = get_dR( mups.at(0).p(), bwdjets[0]);
      if (mums.size() > 0) bwdjet_dr = min(bwdjet_dr, get_dR( mums.at(0).p(), bwdjets[0]));
    }
