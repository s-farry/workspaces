...
ttf   = TFile.Open("/hepstore/sfarry/Powheg/ttbar/llj_ttbar_powheg_13tev.root")
t     = ttf.Get("ttbar")
...
def get_templates(name, t, weights = True):
    fwd = MWTemplate(name, t, fwdjet20)
    for v in variables:
        var = v.var
...
def get_asymmetries(name, t, weights = True):
    fwdasy = MWTemplate(name+"_fwdasy", t, fwdjet20 + fwdasy)
    fwdasy.AddVars(fwdvariables)
    fwdasy.ApplyCut()
    if weights:
        for key, val  in ws.iteritems():
            fwdasy.AddWeight(key, val)
    fwdasy.FillVars()
    fwdasy.Scale(sf)
    fwdasy.ScaleAllWeights(sf)
    fwdasy.SaveToFile()

    bwdasy = MWTemplate(name+"_bwdasy", t, fwdjet20 + bwdasy)
    bwdasy.AddVars(fwdvars)
    bwdasy.ApplyCut()
    if weights:
        for key, val  in ws.iteritems():
            bwdasy.AddWeight(key, val)
    bwdasy.FillVars()
    bwdasy.Scale(sf)
    bwdasy.ScaleAllWeights(sf)
    bwdasy.SaveToFile()
...
output = TFile("/user2/sfarry/workspaces/top/tuples/ttbar_13tev_powheg_predictions_filter.root", "RECREATE")
ptj.write()
etaj.write()
lp_pt.write()
lp_eta.write()
