...
int main(int argc, char* argv[]) {
  ...
  std::map<string, std::map<string, double> >::iterator im;
  ...
  TTree* tree = new TTree("dijet", "jj Tree", 1);
  ...
  while ( maxEvts == -1 || iEvent < maxEvts ) {
    ...
    for (int j = 0 ; j < (int)jets.size() ; ++j){
      if (jets.at(j).eta() >2.2 && jets.at(j).eta() < 4.2) {
	fwdjets.push_back(jets.at(j));
      }
      else if (jets.at(j).eta() < -2.2 && jets.at(j).eta() > -4.2 ) {
    ...
    if (reduce_size){
      if (fwdjets.size() > 0 || bwdjets.size() > 0) tree->Fill();
    }
    else{
      tree->Fill();
    }
