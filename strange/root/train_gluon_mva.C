...
void train_gluon_mva(){

  TFile* outputFile = TFile::Open("tmva_gluon.root", "RECREATE");
  TMVA::Factory *factory = new TMVA::Factory("gluon", outputFile,
  ...
  TCut sig("abs(jet_mc_flavour) < 6   && jet_mc_PT != -1 && jet_PT > 0");
  TCut bkg("abs(jet_mc_flavour) == 21 && jet_mc_PT != -1 && jet_PT > 0");
