...
results Fitter::fit_bjpsik_mass(RooDataSet* ds, string oFile, string cut)
{
  RooRealVar m( fitvar.c_str(), fitvar.c_str(), lo, hi );
  RooPlot* plot = m.frame();

  // Gaussian
  RooRealVar mean( "mean", "mean", 5281, 5000, 5500 );
  ...
  RooAddPdf sum("sum", "crystal ball + gaussian + expo", RooArgList(gauss, expo), RooArgList(Nsig, Nbkg));
  ...
  plot->Draw();
  p->Draw();
  canv->SaveAs( ("BJpsiK_"+oFile+".pdf").c_str() );
  canv->Delete();
...
TObjArray* Fitter::get_mass_v_vars(RooDataSet* ds, string label){
  ...
  for (unsigned int i = 0 ; i < vars.size() ; ++i){
    ...
    for (ir = result_tot.begin() ; ir != result_tot.end() ; ++ir){
      hists1d[(*ir).first] = new TH1F(((*ir).first + "_v_"+var+"_"+label).c_str(),
				 ((*ir).first + "_v_"+var+"_"+label).c_str(),
				 varbins, lo, hi);
    }
    ...
    for (int i = 0 ; i < varbins ; ++i) {
      ...
      results result;
      if (fitmodel == Fitter::jpsi){
	result   = fit_jpsi_mass(ds_reduced   , s1.str(), label3.str());
      }
      else if (fitmodel == Fitter::z0){
	result   = fit_z0_mass(ds_reduced   , s1.str(), label3.str());
      }
      
      for (ir = result_tot.begin() ; ir != result_tot.end() ; ++ir){
	hists1d[(*ir).first]->SetBinContent(i+1, result[(*ir).first].first);
	hists1d[(*ir).first]->SetBinError(i+1, result[(*ir).first].second);
      }
  ...
  for (unsigned int i = 0 ; i < vars2d.size() ; ++i){
    ...
    for (ir = result_tot.begin() ; ir != result_tot.end() ; ++ir){
      hists2d[(*ir).first] = new TH2F(((*ir).first + "_v_"+var1+"_"+var2+"_"+label).c_str(),
				      ((*ir).first + "_v_"+var1+"_"+var2+"_"+label).c_str(),
	...
	//make mass histogram
	ostringstream s1, s2;
	s1<<var1<<"_"<<var2<<"_"<<label<<"_"<<i<<"_"<<j;
	s2<<var1<<"_"<<var2<<"_"<<label<<"_"<<i<<"_"<<j;
	//cut to be presented on plot
	ostringstream label2;
	label2<<binlo1<<" < "<<latexvar1<<" < "<<binhi1<<"\n"<<binlo2<<" < "<<latexvar2<<" < "<<binhi2;
	...
	for (ir = result_tot.begin() ; ir != result_tot.end() ; ++ir){
	  hists2d[(*ir).first]->SetBinContent(i+1, j+1, result[(*ir).first].first);
	  hists2d[(*ir).first]->SetBinError(i+1, j+1, result[(*ir).first].second);
	}
