...
toPlot = {'PT' : 'p_{T} [GeV]', 'ETA': '#eta', 'SPDHits' : 'SPD Hits', 'PHI' : '#phi [rad.]'}

properties = {'location' : '/user2/sfarry/validation/figures/effplots',
              'normalised' : False}
...
#tracking mass
p = Plot([muontrk_2011.Get('TotalHist'),
          muontrk_2012.Get('TotalHist'),
          muontrk_2015.Get('TotalHist')])
p.setProps(properties)
p.setProp('filename', 'trk_mass_years')
p.setProp('xlabel', 'M_{#mu#mu} [GeV]')
p.setProp('ylabel', '[A.U.]')
p.setProp('normalised', True)
p.setProp('colors', None)
p.setProp('linecolors', ['red', 'blue', 'green'])
p.setProp('leglims', [0.2, 0.2, 0.5, 0.4])
p.setProp('labels', ['2011 Data (muon trk t&p)', '2012 Data (muon trk t&p)', '2015 Data (muon trk t&p)'])
p.setProp('markerstyles', [1, 1, 20])
p.setProp('fillstyles', 0)
p.setProp('drawOpts', ['hist', 'hist', 'e1'])
p.setProp('leglims', [0.3, 0.2, 0.7, 0.4])
p.drawROOT()
