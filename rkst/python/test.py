...
dt_bunches = [
    ...
    Bunch(name='b_m'   , var='B_M', bins = 100, lo = 5000, hi = 5500),
...
tp_bunches = [
    ...
    Bunch(name='probe_dR_ext' , var='probe_deltaR', bins = 100, lo = 0, hi = 0.2),
    Bunch(name='probe_dRt_ext' , var='probe_deltaRt', bins = 100, lo = 0, hi = 5000),
    ]
...
sel = TCut("tag_PT > 500 && probe_PT > 500 && probe_TRACK_CHI2NDOF < 1.0 && probe_CaloHcalE/probe_P < 0.1 && probe_IP_OWNPV > 0.1 && B_ENDVERTEX_CHI2/B_ENDVERTEX_NDOF < 2 && tag_ProbNNe > 0.2 && (K_ProbNNk*(1-K_ProbNNp)) > 0.05 && probe_EcalClusterEt/probe_PT > 0.6 && probe_EcalClusterEt/probe_PT < 1.4")
...
for p in true_bunches:
    true_lp.AddVar(p.name, p.var.replace('<l>', 'lp'), p.bins, p.lo, p.hi)
true_lp.Run()
...
true_lm.AddTree(y)
for p in true_bunches:
    true_lm.AddVar(p.name, p.var.replace('<l>', 'lm'), p.bins, p.lo, p.hi)
true_lm.Run()
...
tagprobe_true.SetSelCut(trueprobe)
