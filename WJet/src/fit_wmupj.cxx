...
void do_fit(string var){
  ...
  vector<TH1F*> qcd_jetpt_th1fs;
  qcd_jetpt_th1fs.push_back((TH1F*)f->Get("qcd_jetpt0corr/JetIso"));
  qcd_jetpt_th1fs.push_back((TH1F*)f->Get("qcd_jetpt1corr/JetIso"));
  qcd_jetpt_th1fs.push_back((TH1F*)f->Get("qcd_jetpt2corr/JetIso"));
  qcd_jetpt_th1fs.push_back((TH1F*)f->Get("qcd_jetpt3corr/JetIso"));
  qcd_jetpt_th1fs.push_back((TH1F*)f->Get("qcd_jetpt4corr/JetIso"));
  ...
  for (int i = 0 ; i < nbins ; ++i ){
    ...
    TH1F* mc1b = var.find("Jet_PT") == 0 ? qcd_jetpt_th1fs.at(i) : 0;

    if (var.find("Jet_PT") == 0 ) mc1b->Copy(*mc1);
    ...
    RooDataHist roomc1 = RooDataHist((var+"_roomc1_"+idx).c_str(), "mc1", RooArgSet(x), mc1);
    //RooDataHist roomc1;
    //if (var.find("Jet_PT") == 0 ) roomc1 = RooDataHist((var+"_roomc1_"+idx).c_str(), "mc1", RooArgSet(x), mc1b);
    //else  roomc1 = RooDataHist((var+"_roomc1_"+idx).c_str(), "mc1", RooArgSet(x), mc1);
