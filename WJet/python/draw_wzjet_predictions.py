...
#data_styles = [3004, 3004, 3005, 3005]
...
def normalise_bins(hist):
    if ('TH1' in hist.ClassName()):
        for i in range(hist.GetNbinsX()):
            f = 1.0/hist.GetBinWidth(i+1)
            if (f != 1.0):
                hist.SetBinContent(i+1, hist.GetBinContent(i+1)*f)
                hist.SetBinError(i+1, hist.GetBinError(i+1)*f)
    elif ('TGraph' in hist.ClassName()):
...
def shift_noxerr(graph, size = 0.5, bin = -1):
    x = ROOT.Double(0.0)
    y = ROOT.Double(0.0)
    N = graph.GetN()
    if bin == -1 : r = range(N)
...
def shiftx(graph, size = 0.5, bin = -1):
    x = ROOT.Double(0.0)
    y = ROOT.Double(0.0)
    N = graph.GetN()
    if bin == -1 : r = range(N)
...
leg = ROOT.TLegend(0.65, 0.57, 0.95, 0.82)
#leg.SetNColumns(2)
leg.SetFillStyle(0)
leg.SetBorderSize(0)
...
lhcb = ROOT.TPaveText(0.5, 0.85, 0.85, 0.94, "NDC")
lhcb.AddText("LHCb, #sqrt{s} = 8 TeV")
lhcb.SetFillStyle(0)
lhcb.SetBorderSize(0)

ratio = ROOT.TPaveText(0, 0.1, 0.1, 0.3, "NDC")
ratio.AddText("Ratio")
ratio.SetFillStyle(0)
ratio.SetBorderSize(0)
...
lhcb_shiftleft = ROOT.TPaveText(0.2, 0.8, 0.55, 0.95, "NDC")
...
wj_leg2a = ROOT.TLegend(0.56, 0.75, 0.95, 0.87)
wj_leg2a.SetFillStyle(0)
#wj_leg2a.SetBorderSize(1)
#leg_a.SetLegendTitle('W+')
...
#header.SetTextSize(0.035)
#header.SetTextColor(ROOT.kOrange)
wj_leg2b = ROOT.TLegend(0.56, 0.63, 0.95, 0.75)
wj_leg2b.SetNColumns(2)
wj_leg2b.SetFillStyle(0)
#wj_leg2b.SetBorderSize(1)
#leg_a.SetLegendTitle('W+')
...
zplots.append(Plotter('etaj', j.Get('z_etaj_toterr'), j.Get('z_etaj_staterr'), i.Get('etaj_scaleerrs'), i.Get('etaj_toterrs'), h.Get('etaj_scaleerrs'), h.Get('etaj_toterrs'), 
                      '#it{#eta}^{jet}', 'd#sigma(#it{Zj})/d#it{#eta}^{jet} [pb]', ylims = [0.001,6.8], leg = leg))
zplots.append(Plotter('ptj' , j.Get('z_ptj_toterr') , j.Get('z_ptj_staterr') , i.Get('ptj_scaleerrs'), i.Get('ptj_toterrs'), h.Get('ptj_scaleerrs'), h.Get('ptj_toterrs'),
                      '#it{p}_{T}^{jet} [GeV]','d#sigma(#it{Zj})/d#it{p}_{T}^{jet} [pb/GeV]', leg = leg))
zplots.append(Plotter('yz'  , j.Get('z_y_toterr')  , j.Get('z_y_staterr')  , i.Get('yz_scaleerrs') , i.Get('yz_toterrs') , h.Get('yz_scaleerrs'), h.Get('yz_toterrs')
                      , '#it{y^{Z}}'    ,'d#sigma(#it{Zj})/d#it{y^{Z}} [pb]', ylims = [0.001, 8.0], leg = leg))
zplots.append(Plotter('dphi'  , j.Get('z_dphi_toterr')  , j.Get('z_dphi_staterr')  , i.Get('dphi_scaleerrs') , i.Get('dphi_toterrs') , h.Get('dphi_scaleerrs'), h.Get('dphi_toterrs'), '|#Delta#it{#phi}| [rad]'    ,'d#sigma(#it{Zj})/d|#Delta#it{#phi} [pb/rad]', ylims = [0.001, 16], leg = leg_shiftleft))
for z in zplots: print z.data_tot.GetYaxis().SetTitleOffset(1.3)
...
wpplots.append(Plotter('etaj', j.Get('wp_etaj_toterr'), j.Get('wp_etaj_staterr'), mm.Get('etaj_plus_scaleerrs'),  mm.Get('etaj_plus_toterrs'), l.Get('etaj_plus_scaleerrs'), l.Get('etaj_plus_toterrs'), 
                      '#it{#eta}^{jet}', 'd#sigma(#it{Wj})/#it{#eta}^{jet} [pb]', leg = leg, ylims = [0.001,80]))
wpplots.append(Plotter('ptj' , j.Get('wp_ptj_toterr') , j.Get('wp_ptj_staterr') , mm.Get('ptj_plus_scaleerrs'), mm.Get('ptj_plus_toterrs'), l.Get('ptj_plus_scaleerrs'), l.Get('ptj_plus_toterrs'),
                      '#it{p}_{T}^{jet} [GeV]','d#sigma(#it{Wj})/d#it{p}_{T}^{jet} [pb/GeV]', leg = leg))
wpplots.append(Plotter('eta'  , j.Get('wp_eta_toterr')  , j.Get('wp_eta_staterr')  , mm.Get('eta_plus_scaleerrs') , mm.Get('eta_plus_toterrs') , l.Get('eta_plus_scaleerrs'), l.Get('eta_plus_toterrs')
                      , '#it{#eta^{#mu}}'    ,'d#sigma(#it{Wj})/d#it{#eta^{#mu}} [pb]', leg = leg, ylims = [0.001, 60]))
...
wmplots.append(Plotter('etaj', j.Get('wm_etaj_toterr'), j.Get('wm_etaj_staterr'), mm.Get('etaj_minus_scaleerrs'),  mm.Get('etaj_minus_toterrs'), l.Get('etaj_minus_scaleerrs'), l.Get('etaj_minus_toterrs'), 
                      '#it{#eta}^{jet}', 'd#sigma(#it{Wj})/#it{#eta}^{#it{j}} [pb]', leg = leg, ylims = [0.001, 60]))
wmplots.append(Plotter('ptj' , j.Get('wm_ptj_toterr') , j.Get('wm_ptj_staterr') , mm.Get('ptj_minus_scaleerrs'), mm.Get('ptj_minus_toterrs'), l.Get('ptj_minus_scaleerrs'), l.Get('ptj_minus_toterrs'),
                      '#it{p}_{T}^{jet} [GeV]','d#sigma(#it{Wj})/d#it{p}_{T}^{#it{j}} [pb]', leg = leg))
wmplots.append(Plotter('eta'  , j.Get('wm_eta_toterr')  , j.Get('wm_eta_staterr')  , mm.Get('eta_minus_scaleerrs') , mm.Get('eta_minus_toterrs') , l.Get('eta_minus_scaleerrs'), l.Get('eta_minus_toterrs')
                      , '#it{#eta^{#mu}}'    ,'#sigma(#it{Wj})/d#it{#eta^{#mu}} [pb]', leg = leg, ylims = [0.001, 36]))
...
#    wplots.append(DiPlotter(p.name, p,m, leg = [wj_leg, wj_box1, wj_box2]))
    wplots.append(DiPlotter(p.name, p,m, leg = [lhcb, wj_leg2a, wj_leg2b, wj_box1, wj_box2]))
...
for p in zplots:
    ...
    if binsize > 0:
        ...
        normalise_bins(p.mcatnlo_tot)
        normalise_bins(p.powheg_tot)
        normalise_bins(p.mcatnlo_scale)
        normalise_bins(p.powheg_scale)
        normalise_bins(p.data_tot)
        normalise_bins(p.data_stat)
    ...
    a.properties['ycomplims'] = [0.51,1.49]
    if p.name=='dphi':
        a.properties['ycomplims'] = [0.41, 1.59]
    a.properties['filename']   = "zj_"+p.name+"_8TeV_log"
    a.properties['yloglims']  = [ 0.001, 10]
...
for p in wpplots:
    ...
    #shift just final bin for eta
    if scenario == 3:
        #shift just final bin for eta
        if p.name == 'eta':
            shiftx(p.mcatnlo_tot, -0.55, 3)
            shiftx(p.mcatnlo_scale, -0.55, 3)
            shiftx(p.powheg_tot, -0.2, 3)
            shiftx(p.powheg_scale, -0.2, 3)
    elif scenario == 2:
    #1: shift all w minus left
        shift_noxerr(p.mcatnlo_tot ,size = -0.25)
        shift_noxerr(p.powheg_tot  ,size = -0.55)
        shift_noxerr(p.mcatnlo_scale ,size = -0.25)
        shift_noxerr(p.powheg_scale  ,size = -0.55)
    if scenario == 1 or scenario == 3:
        #1: shift mcatnlo right, powheg left
        shift_noxerr(p.mcatnlo_tot ,size = 0.3)
        shift_noxerr(p.powheg_tot  ,size = -0.3)
        shift_noxerr(p.mcatnlo_scale ,size = 0.3)
        shift_noxerr(p.powheg_scale  ,size = -0.3)
...
for p in wmplots:
    ...
    if scenario == 3:
        #shift just final bin for eta
        if p.name == 'eta':
            shiftx(p.mcatnlo_tot, 0.2, 3)
            shiftx(p.mcatnlo_scale, 0.2, 3)
            shiftx(p.powheg_tot, 0.55, 3)
            shiftx(p.powheg_scale, 0.55, 3)
    if scenario == 2:
        #2: shift all w plus right
        shift_noxerr(p.mcatnlo_tot ,size = 0.55)
        shift_noxerr(p.powheg_tot  ,size = 0.25)
        shift_noxerr(p.mcatnlo_scale ,size = 0.55)
        shift_noxerr(p.powheg_scale  ,size = 0.25)
...
for w in wplots:
    ...
    a.properties['colors'] = [data_colors[1], data_colors[0], data_colors[3], data_colors[2], mc_lcolors[0], mc_lcolors[1], mc_lcolors[2], mc_lcolors[3], mc_lcolors[0], mc_lcolors[1], mc_lcolors[2], mc_lcolors[3], data_colors[1]]
    ...
    a.setProp('ycomplims', [0.71, 1.29])
    if w.name=='etaj': a.setProp('ycomplims', [0.61, 1.59])
    if w.name=='ptj': a.setProp('ycomplims', [0.61, 1.59])
    a.setProp('canvas_size', [600, 800])
    if w.name == 'eta' :
        h = w.plotterB.data_tot
        b = ROOT.TBox(h.GetBinLowEdge(4), h.GetBinContent(4) - h.GetBinError(4),
                      h.GetXaxis().GetBinUpEdge(4), h.GetBinContent(4) + h.GetBinError(4))
        b.SetFillStyle(0)
        b.SetLineColor(h.GetFillColor())
        #a.properties['extraObjs'] += [b]
        a.plots.insert(4, b)
        a.properties['drawOpts'].insert(4, '')
        a.properties['colors'].insert(4,None)
        a.properties['toCompare'] = OrderedDict([(2, [2,3,6+1,7+1,10+1,11+1]), (0, [0, 1, 4+1 ,5+1, 8+1, 9+1])])
    a.drawROOT()
    a.setProp("logscale", True)
    a.setProp("filename", a.properties['filename']+'_log')
    if w.name=='ptj' : a.setProp('ylims', [0.05, 5])
...
a.properties['ycomplims'] = [-0.1,0.1]
#a.properties['ycomplims'] = [0.81,1.19]
#a.properties['ycomplims'] = [-0.09,0.09]
a.properties['toCompare'] = {0 : [0, 1, 2 ,3, 4]}
...
r.properties['ycomplims'] = [0.71,1.29]
...
a.properties['xlims'] = [2, 4.5]
a.properties['ycomplims'] = [-0.1,0.099]
#a.properties['ycomplims'] = [0.81,1.19]
#a.properties['ycomplims'] = [-0.09,0.09]
a.properties['toCompare'] = {0 : [0, 1, 2 ,3, 4]}
a.setProp('compareAsRatio', False)
